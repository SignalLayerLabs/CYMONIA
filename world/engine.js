...
import {classifyHistory,why as whyEvent} from './observer.js';
import {perceiveResources,perceiveStructures} from './perception.js';
...
function stepSegment(world,from,to){
  advanceEnvironment(world,from,to);const delta=to-from;advanceDecomposition(world,delta,to);
  for(const c of world.citizens){advanceBody(world,c,delta);advanceDisease(world,c,from,to);if(c.alive){ageCognitionDebt(c,delta);decayMemories(c,to);perceiveLocal(world,c,to);if(c.body.health<=0)killCitizen(world,c,'disease_or_biology',to);}}
...
export function publicWorld(world,nowMs=Date.now()){const at=worldMinuteAt(world,nowMs);return {version:world.version,worldId:world.worldId,clock:{...world.clock,worldMinute:Math.max(world.clock.worldMinute,at)},environment:world.environment,citizens:world.citizens.map(c=>{const action=activeAction(world,c),pos=action?positionAt(c,action,at):c.position;return {id:c.id,kind:c.kind,selfName:c.selfName,observerDisplayName:c.kind==='HUMAN_LINKED'?c.observerDisplayName:null,alive:c.alive,birthWorldMinute:c.birthWorldMinute,deathWorldMinute:c.deathWorldMinute,position:pos,body:{hydration:c.body.hydration,calories:c.body.calories,sleepPressure:c.body.sleepPressure,health:c.body.health,ageMinutes:c.body.ageMinutes,temperatureC:c.body.temperatureC,exposure:c.body.exposure||null,reproductiveRole:c.body.reproductiveRole||null,pregnancy:c.body.pregnancy?{dueWorldMinute:c.body.pregnancy.dueWorldMinute}:null,diseases:c.body.diseases.length,injuries:c.body.injuries.length},psychology:c.psychology,knowledge:c.kind==='HUMAN_LINKED'?{count:c.knowledge.filter(k=>k.active!==false).length}:c.knowledge.filter(k=>k.active!==false).map(k=>({concept:k.concept,confidence:k.confidence,provenance:k.provenance})),language:{primitiveSignals:c.language.primitiveSignals,lexicon:c.language.lexicon},knownEntityIds:c.knownEntityIds,relationships:c.relationships,possessions:c.possessions,activeGoal:c.activeGoal,currentAction:action?{id:action.id,type:action.type,purpose:action.purpose,startedWorldMinute:action.startedWorldMinute,endsWorldMinute:action.endsWorldMinute,fromPosition:action.fromPosition,targetPosition:action.targetPosition,targetId:action.targetId,path:action.path||null,physics:action.physics||null}:null};}),objects:world.objects.filter(o=>o.quantity>0).map(o=>({id:o.id,kind:o.kind,material:o.material||null,quantity:o.quantity,massPerUnitKg:o.massPerUnitKg,holderId:o.holderId,position:o.position,condition:o.condition,properties:o.properties||null,provenance:o.provenance})),resourceDeposits:world.resourceDeposits.map(d=>({id:d.id,type:d.type,quantity:d.quantity,position:d.position})),buildings:world.buildings.map(b=>({...b,occupants:world.citizens.filter(c=>c.alive&&Math.hypot(c.position.x-b.position.x,c.position.y-b.position.y)<=1.8).map(c=>c.id)})),organizations:world.organizations,claims:world.claims,projects:world.projects,history:classifyHistory(world).entries.slice(0,200),recentLedger:world.ledger.slice(-80).filter(e=>['PHYSICAL_IMPACT','STRUCTURE_DAMAGED','STRUCTURE_FRACTURED'].includes(e.type)).map(e=>({id:e.id,type:e.type,worldMinute:e.worldMinute,actorId:e.actorId,payload:e.payload,causes:e.causes})),ledgerHead:world.ledgerHead};}
