...
## Autonomous World contributions

Changes to the persistent world must preserve the core contract:

- visible activity comes from canonical world state or recorded events;
- Genesis founders are never deleted;
- humans cannot mint CYM or directly edit another Citizen;
- CymScript remains declarative and cannot execute arbitrary code;
- AI may propose behavior but deterministic code authorizes state mutation;
- new world behavior needs a test in `tests/test_autonomous_world.mjs`, `tests/test_world_store.mjs`, or `tests/test_world_api.mjs`.

Run the complete verification before opening a PR:
