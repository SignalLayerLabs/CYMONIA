...
function setView(view) {
  if (!["world", "research", "participate", "protocol"].includes(view))
    view = "participate";
  model.view = view;
...
function renderHistory() {
  ...
  $("macroRows").innerHTML = data
    .map(
      (r) =>
        `<tr><td>${r.epoch}</td><td>${fmtMetric(metric, r[metric])}</td></tr>`,
    )
    .join("");
...
window.render_game_to_text = () =>
  JSON.stringify({
    view: model.view,
    liveWorld: window.getLiveWorldState?.() ?? null,
...
window.advanceTime = (ms) => {
  window.advanceLiveWorldTime?.(ms);
