...
export class ObserverConnection{
  constructor({fetchState,loadReplay=null,createSocket=null,onWorld=()=>{},onMode=()=>{},onError=()=>{},setTimeoutFn=setTimeout,clearTimeoutFn=clearTimeout,randomFn=Math.random,pollMs=12000,minBackoffMs=900,maxBackoffMs=15000}={}){
  ...
  setMode(mode,detail=null){if(this.mode===mode&&detail==null)return;this.mode=mode;this.onMode(mode,detail);}
  ...
  scheduleRetry(){if(this.stopped||this.retryTimer)return;const wait=this.nextBackoff();if(this.lastCanonical)this.setMode(CONNECTION.DEGRADED,{retryInMs:wait});else if(!this.replay)this.setMode(CONNECTION.RECONNECTING,{retryInMs:wait});this.retryTimer=this.setTimeoutFn(async()=>{this.retryTimer=null;if(!this.lastCanonical)this.setMode(CONNECTION.RECONNECTING);await this.refreshNow();},wait);}
