...
try {
 ...
 await page.locator('[data-view="participate"]').click();await page.waitForFunction(()=>document.getElementById('participationStatus').textContent.includes('OFFLINE'));assert.match(await page.locator('#participateView').innerText(),/Can an economy learn/i);assert.match(await page.locator('#participateView').innerText(),/Mission Control could not refresh/i);
 await page.getByRole('button',{name:'Live World',exact:true}).click();await page.locator('[data-market="compute"]').click();await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:output+'/desktop.png',fullPage:true});
 for(const width of [390,768]){
  await page.setViewportSize({width,height:844});
  for(const name of ['Live World','AI laboratory','The protocol']){
 ...
 await page.emulateMedia({reducedMotion:'reduce'});await page.getByRole('button',{name:'Live World',exact:true}).click();assert.equal((await state()).cityPaused,true);
